...
#   client = (pronotepy.ParentClient if type_compte == 'parent' else pronotepy.Client)(url, username, password, ent)
    client = client = (pronotepy.ParentClient if type_compte == 'parent' else pronotepy.Client)('https://0840035g.index-education.net/pronote/mobile.parent.html', username = 'lionel.peres', password='0!&osUflT7xl', ent=atrium_sud)                     
    ...
    message += f" {url} --- {username} ---- {password} --- {ent}"
    print(message)
...
#    print(json.dumps(jsondata['absence'], indent=4))
    ...
    #with open(os.path.join(location, "/config/www/pronote_"+eleve_id+".json"), "a") as outfile:
